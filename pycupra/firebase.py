...
from .const import FCM_PROJECT_ID, FCM_API_KEY, FCM_APP_ID
...
class Firebase:
    ...
    async def firebaseStart(
        ...
        try:
            ...
            fcm_token = (
                await self._pushClient.checkin_or_register()
            ) 
            self._LOGGER.debug(
                f"Firebase.checkin_or_register() returned a token:{fcm_token}"
            )
